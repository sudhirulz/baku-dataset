...
export default function WordScreen({ tick, onDone, hasKey }) {
  ...
  const [wordProgress, setWordProgress] = useState({}) // { entryId: { done, total } }
  ...
  async function runWordAnalysis(entry) {
    ...
    setWordProgress(prev => ({ ...prev, [entry.id]: { done: 0, total: 1, status: 'starting' } }))

    const text = (entry.ocrText || entry.transcription || entry.rawText || '').trim()

    if (!text) {
      // Entry has no text — needs processing first
      setWordProgress(prev => ({ ...prev, [entry.id]: { done: 0, total: 1, status: 'no_text' } }))
      setAnalysing(null)
      return
    }

    setWordProgress(prev => ({ ...prev, [entry.id]: { done: 0, total: Math.ceil(text.split(/\s+/).length / 6), status: 'running' } }))

    try {
      const words = await analyseWords(
        text,
        entry.sourceType,
        entry.zone,
        (done, total) => {
          setWordProgress(prev => ({ ...prev, [entry.id]: { done, total, status: 'running' } }))
        }
      )
      if (words.length > 0) {
      ...
      setWordProgress(prev => ({ ...prev, [entry.id]: { done: 1, total: 1, status: words.length > 0 ? 'done' : 'empty' } }))
    } catch (err) {
      setWordProgress(prev => ({ ...prev, [entry.id]: { done: 0, total: 1, status: 'error', error: err.message } }))
    }

    setAnalysing(null)
    setTimeout(() => {
      setWordProgress(prev => { const n = { ...prev }; delete n[entry.id]; return n })
    }, 3000)
    onDone()
  }
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:6, alignItems:'flex-end' }}>
                  ...
                  {wordProgress[e.id] && (
                    <div style={{ minWidth:180 }}>
                      {wordProgress[e.id].status === 'no_text' && (
                        <div style={{ fontSize:10, color:'var(--red)', fontWeight:500 }}>⚠ No text — process entry first in ② Process tab</div>
                      )}
                      {wordProgress[e.id].status === 'error' && (
                        <div style={{ fontSize:10, color:'var(--red)', fontWeight:500 }}>✕ Error — {wordProgress[e.id].error?.slice(0,40)}</div>
                      )}
                      {wordProgress[e.id].status === 'done' && (
                        <div style={{ fontSize:10, color:'var(--green)', fontWeight:500 }}>✓ Done!</div>
                      )}
                      {wordProgress[e.id].status === 'empty' && (
                        <div style={{ fontSize:10, color:'var(--amber)', fontWeight:500 }}>⚠ No words returned — try again</div>
                      )}
                      {(wordProgress[e.id].status === 'running' || wordProgress[e.id].status === 'starting') && wordProgress[e.id].total > 0 && (
                        <div>
                          <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'var(--text3)', marginBottom:3 }}>
                            <span>Batch {wordProgress[e.id].done}/{wordProgress[e.id].total}</span>
                            <span>{Math.round((wordProgress[e.id].done/wordProgress[e.id].total)*100)}%</span>
                          </div>
                          <div className="progress-track" style={{ height:4 }}>
                            <div className="progress-fill" style={{ width:`${Math.round((wordProgress[e.id].done/wordProgress[e.id].total)*100)}%` }} />
                          </div>
                        </div>
                      )}
                    </div>
